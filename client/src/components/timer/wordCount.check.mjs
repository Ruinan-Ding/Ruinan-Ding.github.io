...
// insert while the text SHRINKS. Both used to skip the cap entirely,
// because the two length tests that guarded it ("shorter, so nothing was
// added" and "it only grew by n") say nothing about lines: a newline
// isn't a character. Measured overruns were 1,000,004 and 1,000,044 lines.
...
// ...and the same trick with a result SHORTER than what it replaced: the
// 50 newlines have to outrun the cap on their own for this to be a test
...
// The deletion half of a replacement still always applies, ceiling or
// not: selecting text and typing over it is how you get back UNDER the
// cap, so a full counter refusing it outright would be a dead end.
assert.equal(
