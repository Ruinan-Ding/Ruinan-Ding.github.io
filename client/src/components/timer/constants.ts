...
// One size in the fullscreen row, on the window rather than on cqi. It
// was two sizes against the room the cluster had left, because that room
// was the only signal that it had run out; now the cluster sits in the
// centred block with the countdown and the row's own ladder drops its
// date and then its time, so what is left to decide is just how big it
// reads, and the answer is the same as everywhere else it is compact.
export const FULLSCREEN_CLOCK_FONT_SIZE = COMPACT_CLOCK_FONT_SIZE;
